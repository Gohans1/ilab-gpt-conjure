...
mock.module("../src/config.js", () => ({ ...config, CHATGPT_LOGIN_URL: server.url.href }));
...
mock.module("../src/check-session.js", () => ({
  ...
  hasValidSessionToken: (cookies: any[]) => cookies.some(c => c.domain === "127.0.0.1" && c.name === "session_probe" && c.value === "local-fixture"),
}));
...
chromium.launchPersistentContext = async (profile, options) => {
  assert(!Array.isArray(options?.ignoreDefaultArgs) || !options.ignoreDefaultArgs.includes("--enable-automation"),
    "Offline capture must retain Playwright's automation mode to avoid Windows de-elevation relaunch");
...
try {
  ...
  if (completion === "done") {
    ...
    const profile = join(root, "login-profiles", readdirSync(join(root, "login-profiles"))[0]);
    closeBrowserGracefully(profile, getActiveBrowserPids()[0]);
  }
  const error = await result;
  if (error) throw error;
  const saved = JSON.parse(readFileSync(config.STORAGE_STATE_PATH, "utf8"));
  assert(saved.cookies.some((c: any) => c.name === "session_probe"), "Session-only cookie was lost");
  assert(saved.cookies.some((c: any) => c.name === "persistent_probe"), "Persistent cookie was lost");
